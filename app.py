...
while cap.isOpened():
    ...
    lable = "Nhan dien cam xuc -Nguyen Hong Phong"
    ...
    try:
        for (x,y, w, h) in faces:
            ...
            emotion_detection = ('Tuc gian', 'Chan ghet', 'So hai', 'Hanh phuc', 'Buon', 'Ngac nhien', 'Tu nhien')
            emotion_prediction = emotion_detection[max_index]
            cv2.putText(res, "Cam xuc: {}".format(emotion_prediction), (0,textY+22+5), FONT,0.7, lable_color,2)
            lable_violation = 'Do chinh xac: {}'.format(str(np.round(np.max(predictions[0])*100,1))+ "%")
